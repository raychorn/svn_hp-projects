...
public abstract class DeleteAdapter<T, S extends BaseModel> extends
		BaseAdapter<T, S> {

	public abstract String getDeleteParams();
	...
	public S call() throws Exception {
		...
		try {
			...
			log.debug(
					"Initialization exception for service "
	...
	String sendRequest(final String urlString) throws NullDataException,
			CommunicationException {
		try {
			HttpDeleteWithBody httpDelete = getHttpDelete(urlString);
			HttpResponse response = httpClient.execute(httpDelete);
			checkHttpResponseForErrors(response);

			HttpEntity entity = response.getEntity();
			String result = getResponse(entity.getContent());
			log.debug("Data returned from the UIM DELETE command for "
					+ this.getServiceName() + " is " + result);
			return checkForUimErrors(result);
		} catch (IOException e) {
			String errorMessage = this.i18nProvider.getInternationalString(
					locale, I18NProvider.Error_NoWsResponse);
			log.debug("httpGet for " + this.getServiceName()
	 ...
	 */
	public HttpDeleteWithBody getHttpDelete(final String urlString)
			throws UnsupportedEncodingException {
	...
	String checkForUimErrors(final String result) throws CommunicationException {
		...
		if (isPythonError(result)) {
			...
			log.error("Recieved a Python exception for "
					+ this.getServiceName()
