package assets.components
{
	...
	public class Hpic4vc_TasksMediator extends Hpic4vc_BaseMediator 
	{
		private var _view:Hpic4vc_Tasks;
		private var _proxy:Hpic4vc_providerProxy;
		private var count:int = 0;
		
		private static var _logger:ILogger = Log.getLogger("Hpic4vc_TasksMediator");
		
		[View]
		/**
		 * The mediator's view.
		 */
		public function get view():Hpic4vc_Tasks {
			return _view;
		}
		
		/** @private */
		public function set view(value:Hpic4vc_Tasks):void {
			_view = value;
		...
		override protected function requestData():void {
			if (_contextObject != null) {
				_logger.debug("Requesting HPIC4VC Tasks data.");
				_proxy.getTaskDetails(_contextObject.uid, onGettingResult, _contextObject);
			} else {
				_logger.warn("ContextObject is null, hence not requesting Tasks data.");
				return;
			}
		}
		...
		private function onGettingResult(event:MethodReturnEvent):void {
			if (_view != null) {
				_logger.debug("Received HPIC4VC Tasks data in onGettingResult()");
				...
				_logger.warn("View is null.  Returning from onGettingResult() for Tasks data.");
				return;
			}
		}
	}
