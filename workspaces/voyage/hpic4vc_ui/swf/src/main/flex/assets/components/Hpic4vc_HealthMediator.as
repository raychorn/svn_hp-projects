package assets.components
{
	...
	public class Hpic4vc_HealthMediator extends Hpic4vc_BaseMediator
	{
		private var _view:Hpic4vc_Health;
		private var _proxy:Hpic4vc_providerProxy;
		private var count:int = 0;
		
		private static var _logger:ILogger = Log.getLogger("Hpic4vc_HealthMediator");
		
		[View]
		/**
		 * The mediator's view.
		 */
		public function get view():Hpic4vc_Health {
			return _view;
		}
		
		/** @private */
		public function set view(value:Hpic4vc_Health):void {
			_view = value;
		...
		override protected function requestData():void {
			if (_contextObject != null) {
				_logger.debug("Requesting HPIC4VC health details data.");
				_proxy.getHealthDetails(_contextObject.uid, onGettingResult, _contextObject);
			} else {
				_logger.warn("ContextObject is null, hence not health details requesting data.");
				return;
			}
		}
		...
		private function onGettingResult(event:MethodReturnEvent):void {
			if (_view != null) {
				_logger.warn("Received HPIC4VC health details data in onGettingResult()");
				...
				_logger.warn("View is null.  Returning from onGettingResult() for health details.");
				return;
			}
		}
	}
