package assets.components
{
	...
	public class Hpic4vc_Storage_VirtualDisksMediator extends Hpic4vc_BaseMediator
	{
		private var _view:Hpic4vc_Storage_VirtualDisks;
		private var _proxyStorage:Hpic4vc_storage_providerProxy;
		private var count:int = 0;
		
		private static var _logger:ILogger = Log.getLogger("Hpic4vc_Storage_VirtualDisksMediator");
		
		[View]
		/**
		 * The mediator's view.
		 */
		public function get view():Hpic4vc_Storage_VirtualDisks {
			return _view;
		}
		
		/** @private */
		public function set view(value:Hpic4vc_Storage_VirtualDisks):void {
			_view = value;
		...
		override protected function requestData():void {
			if (_contextObject != null) {
				_logger.debug("Requesting HPIC4VC Virtual Disks data.");
				_proxyStorage.getVirtualDisks(_contextObject.uid, onGettingResult, _contextObject);
			} else {
				_logger.warn("ContextObject is null, hence not requesting Virtual Disks data.");
				return;
			}
		}
		...
		private function onGettingResult(event:MethodReturnEvent):void {
			if (_view != null) {
				_logger.debug("Received HPIC4VC Virtual Disks data in onGettingResult()");
				...
				_logger.warn("View is null.  Returning from onGettingResult() for Virtual Disks data.");
				return;
			}
		}
	}
