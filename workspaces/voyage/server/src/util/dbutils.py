...
def create_db_tables(user = None, password = None, database='ic4vc', host=None, port=None, table_defs = []) :
    if not user:
...
def get_db_host() :
    cfg = config.config()
    return cfg.uimconfig['private_config']['db'].get('ip',None)
...
def get_db_username() :    
    cfg = config.config()
    return cfg.uimconfig['private_config'].get('db', {}).get('username', None)

def get_db_port() :    
    cfg = config.config()
    return cfg.uimconfig['private_config'].get('db', {}).get('port', 5432)    
