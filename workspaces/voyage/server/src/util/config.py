...
class config:
    def __init__(self):
        ...
        # CS 7.3+ imports the password file into thier database and then renames it.
        # The password file should only be used to get the CS REST credential
        if not os.path.isfile(self.pwfile) :
            self.pwfile = self.resource.get_file_url( self.resource.hpcs_path(), 'share/password.xml.imported' )
