...
class UIManager:
    ...
    def create_task(self, obj_path, task_name, task_name_arg, start_time, status, user_name, task_detail, task_detail_arg):
        data = {}
        data['taskName'] = task_name
        data['taskNameArguments'] = task_name_arg
        data['startTime'] = start_time 
        data['completedTime'] = None
        data['status'] = status
        data['userName'] = user_name
        data['taskDetails'] = task_detail
        data['taskDetailArguments'] = task_detail_arg
        
        uri = self._make_uri('tasks') + "&" + obj_path
        response, rsp_data = self.rest.post(uri, json.dumps({'tasks' : [data]}))
        self._check_server_response(response, rsp_data)
        rsp_data = json.loads(rsp_data)
        return rsp_data

    def update_task(self, obj_path, taskid, task_name, task_name_arg, start_time, completed_time, status, user_name, task_detail, task_detail_arg):
        data = {}
        data['taskName'] = task_name
        data['taskNameArguments'] = task_name_arg
        data['startTime'] = start_time 
        data['completedTime'] = completed_time
        data['status'] = status
        data['userName'] = user_name
        data['taskDetails'] = task_detail
        data['taskDetailArguments'] = task_detail_arg
        
        uri = self._make_uri('tasks') + "&" + obj_path + "&" + 'taskId=' + taskid
        response, rsp_data = self.rest.post(uri, json.dumps(data))
        self._check_server_response(response, rsp_data)
        rsp_data = json.loads(rsp_data)
        return rsp_data
