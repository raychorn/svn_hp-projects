'''
$Author: yanfei.shi@hp.com $
$Date: 2013-10-14 08:12:13 -0500 (Mon, 14 Oct 2013) $
$Header: https://svn02.atlanta.hp.com/local/ic4vc-dev/server/trunk/src/portlets/network.py 5942 2013-10-14 13:12:13Z yanfei.shi@hp.com $
$Id: network.py 5942 2013-10-14 13:12:13Z yanfei.shi@hp.com $
$Revision: 5942 $
'''
'''
...
class Network:
    ...
    def get_network_data(datacollector):
        ...
        vcm={}
        dvss = Network.get_dvss(datacollector,vcm)
        vss = Network.get_vss(datacollector,vcm)
        ...
        with open('data.txt', 'w') as outfile:
            json.dump(raw_vcm, outfile)
        log.debug("HPCS7.3DATA!!!%s",json.dumps(raw_vcm, sort_keys = True, indent = 4,
                                separators = (',', ': ')))
        if not raw_vcm :
            #####################################################################################
            import networkhpov
            data=networkhpov.get_data()
            #####################################################################################
    ...
    @staticmethod    
    def get_dvss(datacollector,vcm) :
        ...
        try : 
            ...
            if datacollector.dvpg:   
                dvpgs = [ x for x in datacollector.dvpg 
                          if (hasattr(x, 'host') and (hasattr(x.host, 'ManagedObjectReference') and 
                                        ...
                                        for pnic in datacollector.host_detail.config.network.pnic :
                                            if spec.pnicDevice == pnic.device :
                                                ...
                                                if datacollector.vcm :                                                
                                                    ...
                                                    for pnic in vcm.get('physical_nics',[]) if (vcm) else []:
                                                        if this_pnic.mac==pnic['mac'] and this_pnic['speedGb']==0:
                                                            this_pnic['physicalPortMapping']=pnic['physicalPortMapping']
                                                    for en in vcm.get('enclosures',[]) if (vcm) else []:
                                                        for sw in en['allVcModuleG1s']:
                                                            for net in sw['networks']:
                                                                for dl in net['downlinks']:
                                                                    try:
                                                                        if dl['macAddress']==this_pnic.mac and dl['physicalPortMapping']:
                                                                            this_pnic['physicalPortMapping']=dl['physicalPortMapping']
                                                                            this_pnic['speedGb']=dl['speedGb']
                                                                            
                                                                                    
                                                                            
                                                                    except:
                                                                        log.debug("MISSING physicalPortMapping !!!")
    ...
    def organize_vcm(datacollector):
        ...
        try:
            ...
            if not externalSwitches:
                for item in vcm.get('telemetry',{}).get('item',[]):
                    if not item:
                        continue
                    port_id=item['id']
                    for target in item.get('data',{}).get('target',[]):
                        p=target.get('instance',{}).get('properties',{})
                        if not p:
                            continue
                        log.debug("SWITCH TYPE!!!%s",p['type'])
                        if p['type']!='external':
                            continue
                        switch_id=p['chassis_id']
                        if switch_id:
                            switch_id=switch_id.lower()
                        sw={}
                        sw['id'] = switch_id
                        sw['remote-chassis-id'] = switch_id          
                        sw['remote-system-desc'] = p['system_desc']          
                        sw['remote-system-name'] = p['system_name']
                        sw['remote-system-capabilities'] = p['system_capabilities'] 
                        sw['remote_chassis_id'] = switch_id          
                        sw['remote_system_desc'] = p['system_desc']          
                        sw['remote_system_name'] = p['system_name']
                        sw['remote_system_capabilities'] = p['system_capabilities']            

                        sw['ports'] = []

                        port = {}
                        port['id'] = port_id
                        port['remote-port-id'] =p['port_id']
                        port['remote-port-desc'] = p['port_desc']
                        # NGC specific as java can't handle hyphenated properties
                        port['remote-port-id'] = p['port_id']
                        port['remote-port-desc'] = p['port_desc']
            ...
            log.debug('EXTERNAL SWITCHES!!!%s',externalSwitches)
