...
class HostFirmware(WBEMClient):
    ...
    def __init__(self, host, vc_uuid, moref, dc):
        self.task_name = 'Firmware Update'
        self.start_time = time.time()
        self.objpath = 'moref=' + moref + '&' + 'serverGuid=' + vc_uuid   
        ...
        try:
            ...
            log.exception('Error getting current session')
        ...
        self.desc = 'Perform firmware update using HP Smart Components on ' + self.host
    ...
    def start_running_jobs_monitor(self):
        self.job_monitor = Thread(name="monitor.firmware_jobs", target=self.monitor_running_jobs)
        self.job_monitor.daemon = True
    ...
    def set_all_task_state(self, jobs, state):
        for job in jobs:
            self.uimgr.update_task(self.objpath, job['taskid'], self.task_name, [], job['start_time'], time.time(), "COMPLETED", self.user, job['desc'],  [])

    def job_found_in_scjobs(self, scjobs, job):
        for scjob in scjobs:
            if job['package_url'] in scjob['Name']:
                return scjob
        return None
        
    def monitor_running_jobs(self):
        self.job_queue_sem.acquire()
        oldrunningjobs = [job for job in self.update_url_queue if not job['skip']]
        self.job_queue_sem.release()
        count = 0
        while count < 15:
            log.info('Staring to monitor the running jobs')
            
            self.job_queue_sem.acquire()
            runningjobs = [job for job in self.update_url_queue if not job['skip']]
            self.job_queue_sem.release()
            scjobs = self.get_scjobs()
            jobfound = None
            if scjobs and runningjobs:
                for job in runningjobs:
                    jobfound = self.job_found_in_scjobs(scjobs, job)
                    if jobfound:
                        if jobfound['JobState'] > 6:
                            log.info(job)
                            log.info(jobfound)
                            log.info( 'Setting task completed because JobState is > 6' )
                            self.uimgr.update_task(self.objpath, job['taskid'], self.task_name, [], job['start_time'], time.time(), "COMPLETED", self.user, job['desc'],  [])
                    else:
                        log.info(job)
                        log.info( 'Setting task completed because job was not found in the scjobs'  )
                        self.uimgr.update_task(self.objpath, job['taskid'], self.task_name, [], job['start_time'], time.time(), "COMPLETED", self.user, job['desc'],  [])
            else:
                log.info( 'Setting all tasks completed because no running jobs found in scjobs list' )
                log.info(runningjobs)
                if runningjobs:
                    self.set_all_task_state(runningjobs, 'COMPLETED')
                self.set_all_task_state(oldrunningjobs, 'COMPLETED')
                break
    ...
    def get_firmware_jobs(self):
        ...
        if instances and self.SC_JOB_CLASS in instances:
            ...
            for job in jobs:
                ...
                # Do not return Reboot Required messages as an error
                log.debug("Firmware Job State: %s, Error Code: %s, Status: %s, Description: %s", job_obj.get('JobState','N/A'), job_obj.get('ErrorCode', 'N/A'), job_obj.get('Status', 'N/A'), job_obj.get('StatusDescriptions', 'N/A'))
                if job_obj['JobState'] == 10 and job_obj['ErrorCode'] == 1 :
                    log.debug("Converting Reboot Required error message")
                    job_obj['JobState'] = 7
                    job_obj['ErrorCode'] = 0
                    job_obj['ErrorDescription'] = 'OK'
                    job_obj['Status'] = 'Reboot Required'
                    job_obj['StatusDescriptions'] = ['The installation of the deliverable was successful.  A reboot is required for the deliverable to be enabled.']
                    job_obj['PercentComplete'] = 100
    ...
    def queue_firmware_job(self, filename, options):
        ...
        if parse_result.scheme != 'http':
            if filename in get_scm().get_sc_file_list():
                cfg = config()
                uim_root = cfg.get_uim_root()
                host = urlparse(uim_root).hostname                

                package_url = 'http://'+host+':63000/static/sc_share/'+filename
            else:
        ...
        if not job:
            self.job_queue_sem.acquire()
            taskid = self.uimgr.create_task(self.objpath, self.task_name, [], job_obj['start_time'], "QUEUED", self.user, job_obj['desc'],  [])
            job_obj['taskid'] = taskid['ids'][0]
            ...
            if not job['run'] and not job['skip']:
                try:
                    ...
                    if result and 'status' in result:
                        ...
                        self.uimgr.update_task(self.objpath, job['taskid'], self.task_name, [], job['start_time'], time.time(), "ERROR", self.user, job['desc'],  [])
                    ...
                    # The job was created successfully.
                        ...
                        self.uimgr.update_task(self.objpath, job['taskid'], self.task_name, [], job['start_time'], None, "RUNNING", self.user, job['desc'],  [])
                        job['run'] = True
                        self.start_running_jobs_monitor()
                        self.job_queue_sem.release() 
                        i+=1
                    else:
    ...
    def delete_queued_job(self, job_url):
        ...
        for job in self.update_url_queue:
            ...
            if job['package_url'] == job_url and not job['run']:
                job['skip'] = True
                found=True
                self.uimgr.update_task(self.objpath, job['taskid'], self.task_name, [], job['start_time'], time.time(), "CANCELED", self.user, job['desc'],  [])
                break
        self.job_queue_sem.release()
        if not found :
            return None
