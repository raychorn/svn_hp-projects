...
class ALDeploymentConnector:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.login()
            if self.sk:
                log.debug("calling discoverDeploymentServer")
                self.discoverDeploymentServer()
                log.debug("Deployment server adapter created.")

        except Exception as err:
            self.dc_status = DC_STATUS_UNKNOWN
            log.exception("Error creating Deployment Connector")
    ...
    def _dc_error(self, code):
        try :
            if str(code) in DC_ERRORS :
                log.error("Deployment Connector Error: %s - %s", code, DC_ERRORS[code])
            else :
                log.error("Unknown Deployment Connector Error %s", code)
        except :
            log.exception("Error logging Deployment Connector Error")
            pass
        ...
        def new():
            ...
            if dc:
                ...
                if dc[0].deployment_running():
                    return
                dc[0].destroy()
            log.debug("Creating ALDeploymentConnector()")
    ...
    def discoverDeploymentServer(self):
        log.debug("discoverDeploymentServer(), calling getJobFolders()")
        self.getJobFolders()
        log.debug("discoverDeploymentServer(), calling getManagedNodes()")
    ...
    def getJobFolders(self, refresh=False):
        ...
        try:
            ...
            log.exception("Webfault in getJobFolders")
            return None
    ...
    def _parseNodeDetails(self, details):
        ...
        try:
            ...
            log.exception('error parsing networkInterfaces')

    def getManagedNodes(self, refresh=False):
        if self._client == None or self.sk == None:
            log.error("getManagedNodes no client or no token")
            return None

        if not self.managedNodes or refresh:
            try:
                result = self._client.service.listManagedNodesPerDS(self.sk, self.host, self.ds_type)
                log.debug("getManagedNodes listManagedNodesPerDS returned %s", result.returnCode)
                if result.returnCode == 0:
                    nodeList = result.nodeList
                    log.debug("getManagedNodes %s nodes returned", len(result.nodeList))
                ...
                log.exception('error getManagedNodes')
                pass
    ...
    def getManagedNodeDetails(self, uuid):
        ...
        try:
            ...
            log.exception('getmanagedNodeDetail failed')
            return None
    ...
    def reconfigure(self, uuid, personalization, os_type):
        ...
        try:
            ...
            log.exception('WebFault in reconfigre')
            node['overall_job_status'] = 1            
            return None
    ...
    def submitFolder(self, folderName, uuid):
        ...
        try:
            ...
            log.exception("WebFault in sumbitFolder")
            return None
  ...
  <nic id="%s">
    ...
    def getScheduleStatusX(self, schedule_ids):
        ...
        try:
            ...
            log.exception('WebFault in getScheduleStatusX')            
            return None
    ...
    def deploy_esxi(self, uuid, folderName, deployment_obj, vcUuid):
        log.info("Deploying to esx to node: %s", uuid)
        node = self._get_node(uuid)
        ...
        if deployment_obj['personalization']:
            log.info("ESXi: Reconfiguring node %s", uuid)
        ...
        if result and result.returnCode == 0:
            ...
            while has_jobs:
                ...
                try:
                    ...
                    log.error('Exception in ESXi monitoring loop: %s' % (str(ex)))
                    exceptionCount += 1
                    if exceptionCount == 5:
                        log.error("Encountered too many exceptions in ESXi monitor loop. Canceling.")
            ...
            if result:
                log.error("Folder submit for %s failed. Code: %d" % (folderName, result.returnCode))
                job = {}
                job['schedule_id'] = -1
                job['folder_name'] = folderName
                job['finished'] = True
                ...
                log.error("Result is None for folder submit %s" % (folderName))
        ...
        try:
            if 'addToVCenter' in deployment_obj and deployment_obj['addToVCenter']['interface']:
                ...
                if interface != '' and interface != 'DHCP':
                    ...
                    if thumbprint != '':
                        ...
                        if result and result._type == 'Task':
                            ...
                            log.error("Result: %s" % str(result))
                            log.error('Error adding host %s to vCenter. The vCenter server did not start the task. The host must be added manually.', interface)
                        ...
                        log.error("Thumbprint not found. Unable to contact the host at %s. Add to vCenter aborted.", interface)
                        node['add_to_vcenter'] = 'Unable to contact the host at %s. Add to vCenter aborted.' % (interface)
                else:
                    log.error("Invalid interface for node %s: %s", uuid, interface)
                    if interface == 'DHCP':
                ...
                log.error("No interface specified to add to vCenter for node %s", uuid)
            ...
            log.exception("Exception while attempting vCenter add")
            raise ex
    ...
    def deploy_esx(self, uuid, folderName, deployment_obj, vcUuid):
        log.info("Deploying esx to node: %s", uuid)
        node = self._get_node(uuid)
        type = self._findJobType(folderName)
        exceptionCount = 0
        ...
        if result and result.returnCode == 0:
            ...
            while has_jobs:
                ...
                if self.dc_status == DC_STATUS_CONNECTION_LOST:
                    log.error("Connection lost - exiting ESX deployment thread.")
                    return 
                try:
                    status = self.getNodeScheduleStatus(uuid)
                    for status_obj in status:
                    ...
                    log.error('Exception in ESX monitoring loop: %s',str(ex))
                    exceptionCount += 1
                    if exceptionCount == 5:
                        log.error("Encountered too many exceptions in ESX monitor loop. Canceling.")
                        return
                    # keep the loop going to try again
                    has_jobs = True
            ...
            if result:
                ...
                log.debug("Node %s Result is None for folder submit %s", uuid, folderName)

            # Don't add to vCenter if the submitFolder process failed.
            return

        if node['overall_job_status'] != 0:
            log.error("Deployment failed for node %s. Exiting.", uuid)
            return
            
        try:
            # For ESX, the personalization must be sent after the image is deployed and the agent is available.
            if deployment_obj['personalization']:
                log.info("ESX: Reconfiguring node %s", uuid)
                result = self.reconfigure(uuid, deployment_obj['personalization'], type)
                if result.returnCode == 0:
                    id = result.scheduleIds[0]
                    log.info("Reconfigure successful for node %s (schedule ID: %s)", uuid, id)
                    ...
                    while reconfiguring:
                        ...
                        if status:
                            ...
                            if hasattr(status, 'returnCode'):
                                ...
                                log.info("Reconfigure finished for node %s", uuid)
                                reconfiguring = False
                            else:
                                log.debug("Waiting for reconfigure to finish (node %s)",uuid)
                                sleep(30)
        except:
            log.exception('exception in deploy_esx, reconfigure section node %s', uuid)
        ...
        try:
            if 'addToVCenter' in deployment_obj and deployment_obj['addToVCenter']['interface']:
                interface = deployment_obj['addToVCenter']['interface']
                if interface != '' and interface != 'DHCP':
                    log.info("Adding node %s to vCenter using interface %s" , uuid, interface)
                    if deployment_obj['addToVCenter']['username'] == "" and deployment_obj['addToVCenter']['password'] == "":
                        esx_credentials = None
                    else:
                        esx_credentials = (deployment_obj['addToVCenter']['username'], deployment_obj['addToVCenter']['password'])
                ...
                log.error('No interface specified to add to vCenter node %s', uuid)
                node['add_to_vcenter'] = 'No interface specified to add to vCenter.'

        except Exception as ex:
            node['add_to_vcenter'] = 'An error occurred while attempting to add host %s to vCenter. See the log for details.' % (interface)
            log.exception("Exception while attempting to add node %s to vCenter", uuid)
    ...
    def _run_deployment(self, deployment_object):
        ...
        log.info("Running deployment (vCenter: %s)", vcUuid)
        ...
        for obj in deployment_collection:
            ...
            if type == 'ESXi':
                ...
                log.info("ESXi: Attempting to start job folder %s for node %s (reconfigure: %s)", job, uuid, str(obj['personalization']))
                ...
                log.info("ESX: Attempting to start job folder %s for node %s", job, uuid)
        ...
        self.deployment_sem.release()
        log.info("Finished deployment (vCenter: %s)", vcUuid)
