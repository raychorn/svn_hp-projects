    ...
    def do_events(self):
        if self.events_enabled:
            try:
                if self.poll_events():
                    for ev in self.events['meEvent']:
                        ...
                        if ev['eventName'] == 'hpcs.event.ConnectionLost':
                            log.debug('Connection lost to ip=%s, entity=%s, id=%s', ev['eventSourceIP'], ev['eventReferencedDataModel'], ev['eventReferencedManagedElementID'])
                ...
                log.exception('Exception in do_event')
